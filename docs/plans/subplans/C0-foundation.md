...
**Gate: passed.** `tri.F`, `matrix1.F` and `eddyx.F` all compile unmodified; 19
golden cases committed, `--check` clean.
...
**`eddyx.F` was harnessed after all**, and it was cheap. The open question was
whether stubbing `ASX_DATA_MOD` (1154 lines) was worth it; the answer is that
`eddyx.F` reads only twelve arrays, three parameters and one logical off it, all
plain data, so `harness/stubs_asx.f90` is ~70 lines. Given that the solver
goldens had already caught a real off-by-one within minutes, the golden route
was clearly worth the hour.

A caution from doing it. `KARMAN`, `GAMAH` and `BETAH` are declared in *lower
case* at `ASX_DATA_MOD.F:216-219`, in a codebase that is otherwise uppercase. A
case-sensitive search finds them nowhere, and `eddyx.F` has `IMPLICIT NONE` — so
the file briefly looks as though it cannot compile. It compiles fine. Search
case-insensitively before concluding anything about this codebase.
